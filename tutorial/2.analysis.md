...
A `Distribution` object requires an argument to decide how to calculate the feature and several arguments for the [numpy.histogram]().
...
where `ax` decides the specific axes, `bar_args`(default={}) controls properties of bars and is delivered into the [matplotlib.bar]() method to draw the above picture.
...
The `draw_feature_boxes` method uses [boxplot]() to compare values of features among different mosaics.
...
Compare to methods drawing with raw values, the `draw_value_bars` method uses another method to compass values into a single index and use [matplot.bar]() to compare values in different groups of mosaics. For example, we define NNRI and VDRI distributions and compare mean values from two groups of simulated mosaics, as
